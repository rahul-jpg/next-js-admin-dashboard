const Hamburger = ({ isOpen, handleClick }) => {
    return (
        <div className={`p-2 self-start absolute top-2 left-2 sm:top-2`} onClick={handleClick}>
            ...
            <span className={`bg-light-TXC-100 dark:bg-dark-TXC-100 block transition-all duration-300 ease-out h-[2.5px] w-6 rounded-sm ${isOpen ? "-rotate-45 -translate-y-1" : "translate-y-0.5"}`}></span>
        </div>
    )
}
